...
export const createTaxInvoice = async (req: Request, res: Response) => {
  // If invoiceDate is not provided, set it to current date (YYYY-MM-DD)
...
export const downloadTaxInvoice = async (req: Request, res: Response) => {
  try {
    // Fetch invoice details from DB using the correct query
    const invoiceId = parseInt(req.params.id, 10);
    if (isNaN(invoiceId)) {
      return res.status(400).json({ error: 'Invalid invoice id' });
    }
    const { getInvoiceById } = await import('../queries/invoiceQueries');
