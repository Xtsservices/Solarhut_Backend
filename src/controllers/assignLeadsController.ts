  ...
  try {
    ...
    if (!leadId || typeof leadId !== "number") {
      return res
        ...
        .json({
          success: false,
          message: "leadId is required and must be a number",
        });
    }
    if (!employeeId || typeof employeeId !== "number") {
      return res
        ...
        .json({
          success: false,
          message: "employeeId is required and must be a number",
        });
    }
    ...
    try {
      ...
      const updated = await leadQueries.assignLeadToEmployee(
        leadId,
        employeeId,
      );
      if (!updated) throw new Error("Failed to assign lead");

      console.log("✅ ASSIGN LEAD DEBUG - Lead assigned to employee:", {
        leadId,
        employeeId,
      });
      ...
      console.log("✅ ASSIGN LEAD DEBUG - Customer created:", {
      ...
      if (lead.location) {
        ...
        console.log("✅ ASSIGN LEAD DEBUG - Customer location created:", {
          locationId,
      ...
      const parsed = parseCapacity(lead.capacity as any);
      const jobData = {
        job_code: jobCode,
        lead_id: leadId,
        customer_id: customer.id,
        ...
        capacity: parsed?.normalized || lead.capacity || undefined,
      ...
      console.log("✅ ASSIGN LEAD DEBUG - Job created:", {
        jobId: job.id,
      ...
      console.log("✅ ASSIGN LEAD DEBUG - Job assignment created:", {
        jobId: job.id,
        employeeId,
      });
...
export const getAssignment = async (req: Request, res: Response) => {
  try {
    ...
    let employee = null;
    if (lead.assigned_to) {
      employee = await employeeQueries.getEmployeeById(
        lead.assigned_to as number,
      );
      if (employee) {
    ...
    return res.json({
      success: true,
      data: { lead, assignedEmployee: employee, job: jobDetails },
    });
  } catch (error) {
