  ...
  try {
    ...
    try {
      ...
      if (!value.first_name || !value.mobile) {
        throw new Error(
          "First name and mobile are required to create an estimation",
        );
      }

      // Check if we can find an existing customer by mobile
      existingCustomerResult =
        await customerQueries.findCustomerByMobileWithServices(
          value.mobile,
      ...
      if (existingCustomerResult && existingCustomerResult.customer) {
        ...
        console.log(
          `✅ Found existing customer: ${customerId} (${existingCustomerResult.customer.customer_code})`,
        );

        // Check if this is a returning customer for a different service
        if (existingCustomerResult.is_returning_customer) {
          console.log(
            `📊 Returning customer with ${existingCustomerResult.total_services} previous service(s)`,
          );
        }
      } else {
        ...
        const { error: customerError, value: customerValue } =
          autoCustomerSchema.create.validate(customerData, {
            abortEarly: false,
          });
        if (customerError) {
          throw new Error(
        ...
        console.log(
          `✅ Created new customer: ${customerId} (${customer.customer_code})`,
        );
      }
      ...
      const estimation = await estimationQueries.createEstimation(
        value,
        connection,
      );
      const estimationId = estimation.id;
      console.log(
        `✅ Created estimation: ${estimationId} linked to customer: ${customerId}`,
      );
      ...
      try {
        ...
        console.log(
          `✅ Created customer service record for estimation: ${estimationId}`,
        );
        customerServiceCreated = true;
      } catch (serviceError: any) {
        console.warn(
          `⚠️ Customer service creation skipped:`,
...
export const getAllEstimations = async (req: Request, res: Response) => {
  try {
    ...
    if (user.roles?.includes("SuperAdmin") || user.roles?.includes("Admin")) {
      ...
      estimations = await estimationQueries.getEstimationsForEmployee(
        user.id,
        filters,
      );
      accessInfo = {
...
export const getEstimationById = async (req: Request, res: Response) => {
  try {
    ...
    const estimation = await estimationQueries.getEstimationById(
      estimationId,
      includeInactiveFlag,
    );
    if (!estimation) {
    ...
    if (!user.roles?.includes("SuperAdmin") && !user.roles?.includes("Admin")) {
      ...
      const hasAccess = await estimationQueries.hasEstimationAccess(
        user.id,
        estimationId,
      );
      if (!hasAccess) {
        return res.status(403).json({
          success: false,
          message:
            "Access denied: You are not authorized to view this estimation",
        });
      }
    }
...
export const getEstimationsByMobile = async (req: Request, res: Response) => {
  try {
    ...
    if (user.roles?.includes("SuperAdmin") || user.roles?.includes("Admin")) {
      ...
      // Employee: only see estimations for customers they have jobs assigned to
      estimations = await estimationQueries.getEstimationsByMobileForEmployee(
        user.id,
        mobile,
      );
      accessInfo = {
...
export const updateEstimation = async (req: Request, res: Response) => {
  try {
    ...
    const existingEstimation = await estimationQueries.getEstimationById(
      id,
      true,
    );
    ...
    if (!user.roles?.includes("SuperAdmin") && !user.roles?.includes("Admin")) {
      ...
      if (!hasAccess) {
        return res.status(403).json({
          success: false,
          message:
            "Access denied: You are not authorized to update this estimation",
        });
      }
    }

    // Validate update data
    const { error, value } = estimationUpdateSchema.validate(req.body, {
      stripUnknown: true,
    });
    if (error) {
    ...
    const updated = await estimationQueries.updateEstimation(
      id,
      value,
      updatedBy,
    );
    ...
    if (!user.roles?.includes("SuperAdmin") && !user.roles?.includes("Admin")) {
      ...
      if (!hasAccess) {
        return res.status(403).json({
          success: false,
          message:
            "Access denied: You are not authorized to delete this estimation",
        });
      }
    }
...
export const downloadEstimationPDF = async (req: Request, res: Response) => {
  try {
    ...
    if (
      ...
      if (!hasAccess) {
        return res.status(403).json({
          success: false,
          message:
            "Access denied: You are not authorized to download this estimation",
        });
      }
    }
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=estimation-${estimation.id}.pdf`,
    );
...
export const getMyRunningEstimations = async (req: Request, res: Response) => {
  try {
    ...
    const estimations = await estimationQueries.getRunningEstimations(
      user.id,
      isEmployee,
    );
    ...
    res.json({
      ...
      description:
        "All new estimations (without job assigned) and estimations with active jobs",
      statuses_included: [
        "Active (new estimations without jobs)",
        "Site Visit",
...
export const getMyPendingEstimations = async (req: Request, res: Response) => {
  try {
    ...
    const isEmployee =
      !user.roles?.includes("SuperAdmin") && !user.roles?.includes("Admin");
    const estimations = await estimationQueries.getPendingEstimations(
      user.id,
      isEmployee,
    );
...
export const getMyWaitingForApprovalEstimations = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    const isEmployee =
      !user.roles?.includes("SuperAdmin") && !user.roles?.includes("Admin");
    const estimations =
      await estimationQueries.getWaitingForApprovalEstimations(
        user.id,
        isEmployee,
      );
    ...
    res.json({
      ...
      note: isEmployee
        ? "Only shows estimations for jobs you created"
        : "Shows all jobs waiting for approval",
    });
  } catch (error) {
...
export const getMyCompletedEstimations = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    const isEmployee =
      !user.roles?.includes("SuperAdmin") && !user.roles?.includes("Admin");
    const estimations = await estimationQueries.getCompletedEstimations(
      user.id,
      isEmployee,
    );
    ...
    res.json({
      ...
      summary: {
        ...
        waiting_for_approval: {
          ...
          statuses: ["Active"],
          note: isEmployee
            ? "Jobs you created needing approval"
            : "All jobs waiting for approval",
        },
        completed: {
      ...
      total_estimations:
        runningEstimations.length +
        pendingEstimations.length +
        waitingForApprovalEstimations.length +
        completedEstimations.length,
  ...
  try {
    ...
    if (!user.roles?.includes("SuperAdmin") && !user.roles?.includes("Admin")) {
      return res.status(403).json({
        success: false,
        message:
          "Access denied - Only SuperAdmin and Admin users can convert estimations to jobs",
    ...
    const existingJob =
      await estimationQueries.checkExistingJobForEstimation(estimationId);
    if (existingJob) {
    ...
    // Convert estimation to job
    const result = await estimationQueries.convertEstimationToJob(
      estimationId,
      user.id,
    );
    ...
    if (!result.success) {
      ...
      return res.status(statusCode).json({
        ...
        user_message:
          "Conversion failed. Please verify estimation data or contact support.",
  ...
  try {
    ...
    if (user.roles?.includes("SuperAdmin") || user.roles?.includes("Admin")) {
      return res.status(403).json({
        success: false,
        message:
          "SuperAdmin and Admin should use the regular estimation creation endpoint",
      });
    }

    // Validate using employee schema (no GST allowed)
    const { error, value } = employeeEstimationSchema.validate(req.body, {
      abortEarly: false,
    });
    if (error) {
      return res.status(400).json({
    ...
    try {
      ...
      if (value.customer_name && value.mobile) {
        ...
        if (existingCustomerResult && existingCustomerResult.customer) {
          ...
          const customerData = {
            ...
            last_name:
              value.customer_name.split(" ").slice(1).join(" ") || null,
            mobile: value.mobile,
            email: null,
            customer_type: "Individual",
            lead_source: "Estimation Request",
          ...
          console.log(
            `Created new customer from estimation: ${customerId} (${customer.customer_code})`,
          );
        }
      }
      ...
      const estimationId =
        await estimationQueries.createEstimation(estimationData);
      ...
      const approvalRequested =
        await estimationQueries.requestEstimationApproval(
          estimationId,
          user.id,
        );
      ...
      const estimation =
        await estimationQueries.getEstimationById(estimationId);
...
export const getPendingApprovalEstimations = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    if (!user.roles?.includes("SuperAdmin") && !user.roles?.includes("Admin")) {
      return res.status(403).json({
        success: false,
        message:
          "Access denied. Only SuperAdmin and Admin can view pending approval estimations.",
      });
    }
    ...
    res.json({
      ...
      message:
        estimations.length === 0
          ? "No estimations pending approval"
          : `${estimations.length} estimation(s) pending approval`,
    });
  } catch (error) {
  ...
  try {
    ...
    if (!user.roles?.includes("SuperAdmin") && !user.roles?.includes("Admin")) {
      return res.status(403).json({
        success: false,
        message:
          "Access denied. Only SuperAdmin and Admin can approve estimations.",
      });
    }
    ...
    if (!estimationId) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid estimation ID" });
    }

    // Validate approval data
    const { error, value } = estimationApprovalSchema.validate(req.body, {
      abortEarly: false,
    });
    if (error) {
      return res.status(400).json({
    ...
    const approved = await estimationQueries.approveEstimation(
      estimationId,
      value,
      user.id,
    );
    ...
    if (!approved) {
      return res.status(400).json({
        success: false,
        message:
          "Failed to approve estimation. It may have been modified by another user.",
      });
    }
  ...
  try {
    ...
    if (!user.roles?.includes("SuperAdmin") && !user.roles?.includes("Admin")) {
      return res.status(403).json({
        success: false,
        message:
          "Access denied. Only SuperAdmin and Admin can reject estimations.",
      });
    }
    ...
    const { error, value } = estimationRejectionSchema.validate(req.body, {
      abortEarly: false,
    });
    if (error) {
      return res.status(400).json({
    ...
    const rejected = await estimationQueries.rejectEstimation(
      estimationId,
    ...
    if (!rejected) {
      return res.status(400).json({
        success: false,
        message:
          "Failed to reject estimation. It may have been modified by another user.",
      });
    }
...
export const getMyEstimationsByStatus = async (req: Request, res: Response) => {
  try {
    ...
    const estimations =
      await estimationQueries.getMyEstimationsByApprovalStatus(user.id, status);
    ...
    res.json({
      ...
      message:
        estimations.length === 0
          ? `No estimations with status: ${status}`
          : `${estimations.length} estimation(s) with status: ${status}`,
    });
  } catch (error) {
...
export const getEstimationApprovalHistory = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    if (!user.roles?.includes("SuperAdmin") && !user.roles?.includes("Admin")) {
      ...
      if (!estimation || estimation.created_by !== user.id) {
        return res.status(403).json({
          success: false,
          message:
            "Access denied. You can only view approval history of your own estimations.",
        });
      }
    }

    const history =
      await estimationQueries.getEstimationApprovalHistory(estimationId);
