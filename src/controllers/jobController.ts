...
import { jobSchema, jobLocationSchema, jobAssignmentSchema, jobStatusTrackingSchema, jobPaymentSchema, jobNotesSchema, jobStatusAttachmentSchema, autoCustomerSchema } from '../utils/validations';
import { db } from '../db';
    ...
    try {
        ...
        try {
            ...
            if (!customerId && value.customer) {
                ...
                const { error: customerError, value: customerValue } = autoCustomerSchema.create.validate(value.customer, { abortEarly: false });
                if (customerError) {
                ...
                console.log(`Customer handled with ID: ${customerId} (${customer.customer_code})`);
            } else if (customerId) {
            ...
            await customerQueries.createCustomerService({
                customer_id: customerId,
                service_type: value.service_type || 'Solar Installation',
            ...
            if (!locationId && value.location && customerId) {
                ...
                await jobQueries.updateJob(jobId, { location_id: locationId }, user.id, connection);
    ...
    try {
        ...
        try {
            ...
            if (!isSuperAdmin) {
                ...
                params.push(user.id);
                console.log('🔍 LIST JOBS DEBUG - Non-SuperAdmin filter applied for employee:', user.id);
            } else {
            ...
            if (!isSuperAdmin && totalJobs === 0) {
                console.log('⚠️ LIST JOBS DEBUG - Employee has 0 jobs! Checking job_assignments...');
                const [assignmentCheck] = await connection.execute(
                    `SELECT COUNT(*) as total FROM job_assignments WHERE employee_id = ?`,
                    [user.id]
                ) as any;
                console.log('📊 LIST JOBS DEBUG - Job assignments for this employee:', assignmentCheck[0].total);
                
                // Also check if ANY jobs exist at all
                const [allJobsCheck] = await connection.execute(
                    `SELECT COUNT(*) as total FROM jobs`
                ) as any;
                console.log('📊 LIST JOBS DEBUG - Total jobs in system:', allJobsCheck[0].total);
                ...
                console.log('📊 LIST JOBS DEBUG - Assignment statuses in DB:', statusCheck.map((r: any) => r.assignment_status));
            }
            ...
            const [jobsResult] = await connection.execute(jobsQuery, queryParams) as any;
        ...
        const structuredJobs = await Promise.all(jobsWithAssignments.map(async (job: any) => {
            try {
                // Fetch attachments with signed URLs for this job
                const attachments = await statusHistoryUtils.getAllJobAttachments(job.id);
                if (attachments.length > 0) {
                    console.log(`✓ listJobs - Job ${job.id}: fetched ${attachments.length} attachments with details:`, 
                        JSON.stringify(attachments.slice(0, 1), null, 2)); // Log first attachment
                } else {
                    console.log(`✗ listJobs - Job ${job.id}: NO attachments found (check if status updates include files or if s3_key is populated)`);
                }
    ...
    try {
        ...
        try {
            ...
            if (existingJob.status !== 'Active') {
                throw new Error(`Cannot assign employees to job with status "${existingJob.status}". Job must have "Active" status to add assignments.`);
            }
            ...
            await jobQueries.updateJob(value.job_id, { status: 'Site Visit' }, user.id, connection);
            await jobQueries.createJobStatusTracking({
                job_id: value.job_id,
...
export const assignJobToEmployee = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const { error, value } = jobAssignmentSchema.create.validate(requestData, { abortEarly: false });
        if (error) {
            return res.status(400).json({ 
                success: false, 
                message: 'Validation error', 
...
export const updateJobStatus = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (!hasAccess) {
            return res.status(403).json({ 
                success: false, 
                message: 'Access denied. You can only update status of jobs assigned to you.',
                error_code: 'JOB_ACCESS_DENIED'
            });
        }
        ...
        try {
            ...
            if (existingJob.status === 'Job Done' && value.new_status !== 'Job Done') {
                throw new Error(`Job is already completed and marked as done. Cannot change status from a completed job.`);
            }
            ...
            if (value.new_status === 'Partial Payment Done' && value.payment_details) {
                ...
                if (!paymentData.amount || !paymentData.payment_method) {
                    throw new Error('Amount and payment method are required for partial payment');
                }
            ...
            else if (value.new_status === 'Invoice Generated' && value.payment_details) {
                const paymentData = value.payment_details;
                ...
                for (const attachment of value.attachments) {
                    ...
                    if (hasActualFile && !s3Key) {
                        try {
                            ...
                            const fileName = attachment.file.originalname || attachment.file_name;
                            ...
                            console.log(`📤 Uploading actual file from: ${filePathToUpload} → ${s3KeyPath}`);
                            
                            const uploadResult = await uploadToS3(
                                filePathToUpload,
                                s3KeyPath,
                                attachment.mime_type || attachment.file.mimetype || 'application/octet-stream'
                            );
                            ...
                            throw new Error(`Failed to upload attachment: ${(uploadError as any).message}`);
                        }
                    } else if (s3Key) {
                ...
                for (const receipt of value.payment_details.receipt_attachments) {
                    ...
                    if (hasActualFile && !s3Key) {
                        try {
                            ...
                            console.log(`📤 Uploading receipt from: ${filePathToUpload} → ${s3KeyPath}`);
                            
                            const uploadResult = await uploadToS3(
                                filePathToUpload,
                                s3KeyPath,
                                receipt.mime_type || receipt.file.mimetype || 'application/octet-stream'
                            );
                            ...
                            console.error(`❌ Failed to upload receipt to S3: ${uploadError}`);
                            throw new Error(`Failed to upload receipt attachment: ${(uploadError as any).message}`);
                        }
                    } else if (s3Key) {
            ...
            if (value.new_status === 'Job Done') {
                ...
                console.log(`Updated end_date for job assignments of job ${id} to ${today}`);
            }
            ...
            if (estimations[0] && estimations[0].length > 0) {
                ...
                const syncResult = await require('../queries/estimationQueries').syncEstimationStatusWithJobStatus(
                    estimationId,
                    id,
                    value.new_status,
                    connection
                ...
                if (syncResult) {
                    console.log(`Synced estimation ${estimationId} status with job ${id} status: ${value.new_status}`);
                }
            }
            ...
            if (updatedJob && updatedJob.lead_id) {
                ...
                await leadQueries.syncLeadStatusWithJobStatus(updatedJob.lead_id, value.new_status);
                console.log(`Synced lead status for lead ${updatedJob.lead_id} with job status: ${value.new_status}`);
            }
            ...
            if (updatedJob.customer_id) {
                ...
                console.log(`Updated customer service status to: ${serviceStatus} for customer: ${updatedJob.customer_id}`);
            }
            ...
            if (paymentRecord) {
                ...
                switch (value.new_status) {
                    case 'Partial Payment Done':
                        responseMessage = 'Job status updated and partial payment recorded successfully';
                        if (paymentRecord.payment_time) {
                            responseMessage += ` at ${paymentRecord.payment_time}`;
                        }
                        break;
                    case 'Payment Done':
                        responseMessage = 'Job status updated and payment recorded successfully';
                        if (paymentRecord.payment_time) {
                            responseMessage += ` at ${paymentRecord.payment_time}`;
                        }
                        break;
                    case 'Invoice Generated':
                        responseMessage = 'Job completed successfully with final payment recorded';
                        break;
                    default:
                        responseMessage = 'Job status updated with payment recorded';
                }
            } else if (value.new_status === 'Job Done') {
                responseMessage = 'Job marked as done successfully and moved to completed jobs';
            ...
            if (attachmentRecords.length > 0) {
                responseData.attachments = attachmentRecords.map(att => ({
                    ...
                    signed_url: att.s3_key ? `https://your-bucket.s3.amazonaws.com/${att.s3_key}?signature=placeholder` : att.file_path
                }));
            }

            if (receiptAttachments.length > 0) {
                responseData.receipt_attachments = receiptAttachments.map(receipt => ({
                    ...receipt,
                    // Generate S3 signed URL placeholder - implement actual S3 logic  
                    signed_url: receipt.s3_key ? `https://your-bucket.s3.amazonaws.com/${receipt.s3_key}?signature=placeholder` : receipt.file_path
                }));
            }
            ...
            try {
                ...
                console.log(`Status update notification: Job ${id} status changed to ${value.new_status} by ${user.first_name} ${user.last_name}`);
                ...
                const notificationData = {
                    ...
                    priority: ['Job Done', 'Payment Done', 'Invoice Generated'].includes(value.new_status) ? 'high' : 'normal'
                };
...
export const convertLeadToJob = async (req: Request, res: Response) => {
    ...
    try {
        ...
        try {
            ...
            const customer = await customerQueries.createCustomer(customerData, user.id, connection);
            ...
            if (lead.location) {
                ...
                const location = await customerQueries.createCustomerLocation(locationData, user.id, connection);
            ...
            console.log('✅ CONVERT LEAD DEBUG - Job created:', { job_id: job.id, job_code: jobCode, status: 'Active' });
            ...
            if (employeeToAssign) {
                ...
                await jobQueries.createJobAssignment(assignmentData, user.id, connection);
                ...
                // Update job status to Site Visit since we're adding the first assignment
                await jobQueries.updateJob(job.id, { status: 'Site Visit' }, user.id, connection);
                await jobQueries.createJobStatusTracking({
                    job_id: job.id,
                    previous_status: 'Active',
                    new_status: 'Site Visit',
                ...
                console.log('⚠️ CONVERT LEAD DEBUG - No employee assigned, job created without assignment');
            }
...
export const grantJobMonitoringAccess = async (req: Request, res: Response) => {
    try {
        ...
        if (!userRoles.includes('SuperAdmin')) {
            return res.status(403).json({ 
                success: false, 
                message: 'Access denied. Only SuperAdmin can grant job monitoring permissions.',
...
export const revokeJobMonitoringAccess = async (req: Request, res: Response) => {
    try {
        const user = (res.locals as any).user;
        if (!user || !user.id) {
            return res.status(401).json({ success: false, message: 'User information not found' });
        ...
        if (!userRoles.includes('SuperAdmin')) {
            return res.status(403).json({ 
                success: false, 
                message: 'Access denied. Only SuperAdmin can revoke job monitoring permissions.',
                error_code: 'INSUFFICIENT_PRIVILEGES'
            });
        }
...
const checkJobAccess = async (jobId: number, userId: number, userRoles: string[]): Promise<boolean> => {
    try {
        ...
        const isAssigned = jobAssignments.some(assignment => 
            assignment.employee_id === userId && 
            (assignment.assignment_status === 'Active' || assignment.assignment_status === 'Assigned')
        );
...
export const addJobNote = async (req: Request, res: Response) => {
    try {
        ...
        if (!hasAccess) {
            return res.status(403).json({ 
                success: false, 
                message: 'Access denied. You can only add notes to jobs assigned to you.',
                error_code: 'JOB_ACCESS_DENIED'
            });
        }
...
export const getJobNotes = async (req: Request, res: Response) => {
    try {
        ...
        if (!hasAccess) {
            return res.status(403).json({ 
                success: false, 
                message: 'Access denied. You can only view notes for jobs assigned to you.',
                error_code: 'JOB_ACCESS_DENIED'
            });
        }
...
export const getJobAttachments = async (req: Request, res: Response) => {
    try {
        ...
        if (!hasAccess) {
            return res.status(403).json({ 
                success: false, 
                message: 'Access denied. You can only view attachments for jobs assigned to you.',
                error_code: 'JOB_ACCESS_DENIED'
            });
        }
        ...
        const groupedAttachments = attachments.reduce((acc: any, attachment: any) => {
...
export const getStatusAttachments = async (req: Request, res: Response) => {
    try {
        ...
        if (!statusTrackingId) {
            return res.status(400).json({ success: false, message: 'Invalid status tracking ID' });
        }

        // Get attachments for this status update
        const attachments = await jobQueries.getJobStatusAttachmentsByStatusId(statusTrackingId);
        ...
        try {
            ...
            res.status(200).json({
                ...
                debug_info: {
                    ...
                    all_assignment_statuses_in_db: allStatuses.map((r: any) => r.assignment_status),
                    ...
                    query_explanation: {
                        ...
                        check_2: 'Job assignments count - how many jobs are assigned to this employee',
                        check_3: 'Assigned jobs details - list of jobs and their statuses',
                        check_4: 'Jobs matching listJobs filter - jobs that SHOULD appear in /api/jobs/allJobs',
                        check_5: 'All possible assignment statuses - verify correct status values used',
                        check_6: 'Leads assigned to this employee - which leads are theirs',
                        check_7: 'Lead conversion status - which leads were converted to jobs and if assignments exist',
                        check_8: 'Orphaned jobs - jobs created from leads but with NO job_assignments entry (this is the problem!)'
                    },
                    action_if_orphaned_jobs_exist: 'These jobs exist but have no assignments! They need manual assignment via /api/jobs/assignment/create OR lead.assigned_to needs to be set before conversion'
                }
            });
        } finally {
            connection.release();
...
export const fixJobAssignment = async (req: Request, res: Response) => {
    ...
    try {
        ...
        try {
            ...
            if (employee.status !== 'Active') {
                throw new Error(`Employee "${employee.first_name} ${employee.last_name}" is not Active`);
            }
            ...
            const assignmentId = await jobQueries.createJobAssignment(assignmentData, user.id, connection);

            // Update job status to Site Visit if currently Active
            if (job.status === 'Active') {
                await jobQueries.updateJob(job_id, { status: 'Site Visit' }, user.id, connection);
                await jobQueries.createJobStatusTracking({
                    job_id: job_id,
                    previous_status: 'Active',
                    new_status: 'Site Visit',
                ...
                console.log('✅ FIX JOB ASSIGNMENT DEBUG - Job status updated to Site Visit');
            }
            ...
            res.status(201).json({
                ...
                data: {
                    ...
                    job_status_updated: job.status === 'Active' ? 'Active → Site Visit' : job.status
                }
            });
 ...
 */
export const getJobWithCompleteStatusHistory = async (req: Request, res: Response) => {
    try {
        const user = (res.locals as any).user;
        if (!user || !user.id) {
            return res.status(401).json({ success: false, message: 'User information not found' });
        ...
        const jobDetails = {
            ...
            status_history_summary: {
                ...
                latest_status: statusHistory.length > 0 ? {
        ...
        res.json({ 
            success: true, 
            data: jobDetails,
            message: 'Job details with comprehensive status history retrieved successfully'
        });
    } catch (err) {
 ...
 */
export const getJobStatusHistoryDetails = async (req: Request, res: Response) => {
    try {
        const user = (res.locals as any).user;
        if (!user || !user.id) {
            return res.status(401).json({ success: false, message: 'User information not found' });
 ...
 */
export const getJobCompleteAttachments = async (req: Request, res: Response) => {
    try {
        const user = (res.locals as any).user;
        if (!user || !user.id) {
            return res.status(401).json({ success: false, message: 'User information not found' });
