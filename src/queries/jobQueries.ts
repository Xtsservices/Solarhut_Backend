...
export const createJobPayment = async (
    ...
    const [result] = await conn.execute(
        `INSERT INTO job_payments (
            job_id, payment_type, amount, payment_method, payment_status,
            transaction_id, payment_reference, payment_date, due_date,
            milestone_description, receipt_url, created_by
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
            paymentData.job_id, 
