...
export const createEstimation = async (
  estimationData: CreateEstimationData,
  ...
  const [result] = await conn.execute<ResultSetHeader>(
    ...
    [
      ...
      estimationData.customer_name ??
        `${estimationData.first_name} ${estimationData.last_name}`,
...
export const getAllEstimations = async (filters?: {
  status?: string;
  state?: string;
  district?: string;
...
export const getEstimationsForEmployee = async (
  employeeId: number,
  filters?: {
    status?: string;
    state?: string;
...
export const hasEstimationAccess = async (
  employeeId: number,
  estimationId: number,
): Promise<boolean> => {
  const query = `
...
export const getEstimationsByMobileForEmployee = async (
  employeeId: number,
  mobile: string,
) => {
  const query = `
  ...
  const [estimations] = await db.execute<Estimation[]>(query, [
    employeeId,
    mobile,
  ]);
  return estimations;
...
export const getWaitingForApprovalEstimations = async (
  employeeId: number,
  isEmployee: boolean = true,
) => {
  let query = `
...
export const requestEstimationApproval = async (
  estimationId: number,
  employeeId: number,
): Promise<boolean> => {
  const [result] = await db.execute<ResultSetHeader>(
...
export const getPendingApprovalEstimations = async (): Promise<
  Estimation[]
> => {
  const [estimations] = await db.execute<Estimation[]>(
    `SELECT e.*, 
...
export const approveEstimation = async (
  ...
  const [result] = await db.execute<ResultSetHeader>(
    ...
    [approvalData.gst, approvalData.final_amount, approvedBy, estimationId],
  );
  return result.affectedRows > 0;
};
...
export const checkExistingJobForEstimation = async (
  estimationId: number,
  connection?: PoolConnection,
): Promise<number | null> => {
  const conn = connection || db;
...
export const convertEstimationToJob = async (
  ...
  try {
    ...
    if (estimation.job_id) {
      return {
        success: false,
        message: "A job has already been created for this estimation",
        error: `Job ID: ${estimation.job_id}`,
      };
    }
    ...
    if (!customerId) {
      ...
      // Prefer structured name fields; fall back to estimation.customer_name if available
      let custFirst = "";
      let custLast: string | null = null;

      if (estimation.first_name && estimation.first_name.trim()) {
        custFirst = estimation.first_name.trim();
        custLast =
          estimation.last_name && estimation.last_name.trim()
            ? estimation.last_name.trim()
            : null;
      } else if (estimation.customer_name && estimation.customer_name.trim()) {
        const nm = estimation.customer_name.trim();
        const parts = nm.split(/\s+/);
        if (parts.length === 1) {
          custFirst = parts[0];
          custLast = null;
        } else {
          custLast = parts.pop() || null;
          custFirst = parts.join(" ");
        }
      } else {
        custFirst = "";
        custLast = null;
    ...
    if (!country) {
      return {
        success: false,
        message: "Country India not found in database",
    ...
    // Get or create state
    let state = await stateQueries.getStateByName(
      countryId,
      estimation.state,
      conn,
    );
    if (!state) {
      ...
      console.log(
        `[Estimation ${estimationId}] Creating missing state: "${estimation.state}"`,
      );
    ...
    let district = await districtQueries.getDistrictByName(
      stateId,
      estimation.district,
      conn,
    );
    if (!district) {
      ...
      console.log(
        `[Estimation ${estimationId}] Creating missing district: "${estimation.district}" in state "${estimation.state}"`,
      );
      // Generate district code from first 2 letters of district name + incrementing number
      const districtCodeBase = estimation.district
        .toUpperCase()
        .substring(0, 2);
      const [codeResult] = await conn.execute<any[]>(
    ...
    const jobSolarService =
      solarServiceMap[estimation.solar_service] || "Residential Solar";
    ...
    // Create job
    const parsedCapacity = parseCapacity(estimation.product_description);
    const [jobResult] = await conn.execute<ResultSetHeader>(
      `INSERT INTO jobs (
                job_code, customer_id, location_id, service_type, solar_service, lead_id,
                capacity, capacity_raw, capacity_value, capacity_unit, estimation_id, estimated_cost, job_description, status, created_by
      ...
      [
        ...
        // Use product_description for capacity (preferred) and normalize
        parsedCapacity?.normalized || estimation.product_description || null,
        estimation.product_description || null,
        parsedCapacity?.value || null,
        parsedCapacity?.unit || null,
        estimationId,
...
export const syncEstimationStatusWithJobStatus = async (
  estimationId: number,
  jobId: number,
  jobStatus: string,
  connection?: PoolConnection,
